...
function ComparisonTab() {
  ...
  return (
    <div className={styles.container}>
      {!targetPicklist.val && (
        <div className={styles.errorBox}>Choose a picklist to get started</div>
      )}
      {!targetPicklist.val ||
        (comparedTeams.val?.length == 0 && (
          <div className={styles.errorBox}>Choose a team to see its stats</div>
        ))}
      {targetPicklist.val &&
        comparedTeams.val &&
        comparedTeams.val?.length > 0 && (
...
function ComparedTeams() {
  ...
  return (
    ...
    >
      {focusTeams.val
        .sort((a, b) => Number(a.minimized) - Number(b.minimized))
        .map((val) => {
...
function ComparedTeamElement({
  ...
  team: { teamKey: string; minimized: boolean };
}) {
  const focusTeams = useContext(ComparedTeamKeysContext);
  ...
  const teamName =
    useContext(GlobalTeamDataContext).EPAdata[team.teamKey]?.name || "N/A";
  ...
  function handleMinimizeToggle() {
    if (focusTeams.setVal && focusTeams.val) {
      ...
      focusTeams.setVal((prev) => {
        if (!team.minimized) {
          return prev.map((val) =>
            val.teamKey === team.teamKey ? { ...val, minimized: true } : val,
          );
        }
        ...
        const updatedTeams = prev.map((val) => ({
          ...
          minimized:
            maxTeams.length >= 2 &&
            val.teamKey === maxTeams[maxTeams.length - 1].teamKey
              ? true
              : val.minimized,
        }));
        ...
        {team.minimized && (
          ...
          >
            ...
            >
              ...
              >
                <div>
                  <i className={`fa-solid fa-grip ${styles.gripControl}`} />
                </div>
          ...
          {values.map((val) => {
            return (
              ...
              >
                ...
                >
                  {val.value < 1 ? val.value.toFixed(2) : val.value.toFixed(1)}
                </div>
              </div>
            );
          })}
